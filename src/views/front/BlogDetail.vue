    ...
    <el-row :gutter="10">
      <el-col :span="10">
        <el-card style="height: 280px;">
          <img :src="blog.img" :preview-src-list="[blog.img]"
      ...
      <el-col :span="14">
        <el-card style="height: 280px">
          <el-row>
            <el-col :span="14">
              <div class="item1" style="margin: 15px 0 0 25px"><h2 style="color: #E6A23C">{{ blog.name }}</h2></div>
            </el-col>
            <el-col :span="8">
              <div style="margin-top: 15px;display: flex">
                  <span v-if="state" style="margin-left: 100px;"><img src="../../assets/喜欢.png" alt="" style="width: 25px; position: relative; top: 10px;"  @click="collect" /></span>
                  <span v-if="!state" style="margin-left: 100px;"><img src="../../assets/喜欢 (1).png" alt="" style="width: 25px; position: relative; top: 10px;" v-if="!state" @click="collect" /></span>

                  <span  style="margin-left: 30px;color: #F56C6C"><img src="../../assets/浏览量.png" alt="" style="width: 30px; position: relative; top: 10px;" />&nbsp;{{pageviews}}</span>
                </div>
            </el-col>

          </el-row>

          <div class="item1" style="font-size: 17px; margin: 0 0 0 30px;">
            <span style="color: #666666">新闻类别</span>&nbsp;&nbsp;:&nbsp;&nbsp;<span style="color: #222">{{blog.blogType }}</span>
          </div>
          <div class="item1" style="font-size: 17px; margin: 0 0 0 30px;">
  ...
  methods: {
    collect(){
      ...
      this.request.post("/collect", data).then(res => {
        if (res.code === '200') {
          this.state = false;
          this.$message.success("收藏成功",2000)
        } else {
          this.state = true;
    ...
    saveReply() {
      ...
      this.request.post("/comment", this.replyComment).then(res => {
        if (res.code === '200') {
          this.$message.success("评论成功",2000)
          this.replyComment = {}
          this.dialogFormVisible3 = false
          this.load2()
        } else {
          this.$message.error("保存失败",2000)
        }
      })
    },
    submit2() {
